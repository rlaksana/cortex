import('./dist/services/memory-store.js')
  .then(async ({ memoryStore }) => {
      ...
      },
      {
        ...
        item: {
          ...
          data: {
            ...
            rationale:
              'PostgreSQL provides superior ACID compliance, advanced JSON support, and robust indexing options. This decision ensures data integrity and scalability.',
            status: 'accepted',
          },
        },
      },
        ...
        item: {
          ...
          data: {
            ...
            bug_fixes: [
              ...
              'Corrected field mapping problems',
            ],
          },
        },
      },
      {
        ...
        item: {
          ...
          data: {
            ...
            ddl_text:
              'CREATE TABLE knowledge_items (id UUID PRIMARY KEY DEFAULT gen_random_uuid());',
            checksum: crypto
              .createHash('sha256')
              .update(
                'CREATE TABLE knowledge_items (id UUID PRIMARY KEY DEFAULT gen_random_uuid());'
    ...
    if (finalSuccessCount === 16) {
      ...
      console.log(
        `🔧 Issues found: Database field mapping, UUID validation, trigger configurations`
      );
    }

    console.log('\\n🎯 SUMMARY:');
    console.log(
      `✅ Core Knowledge Types: Section, Decision, Issue, Todo, Runbook, Change, Incident, Release, Assumption`
    );
