/**
 * Production Z.AI Configuration Management
 *
 * Production-ready configuration for Z.AI services with comprehensive
...
export class ProductionZAIConfigManager {
    constructor(environment) {
        ...
        if (!productionConfigs[this.environment]) {
            logger.warn({ environment: this.environment }, 'Unknown environment, falling back to development');
    ...
    validateConfig() {
        ...
        if (this.config.ai.enabled && !this.config.zai.url) {
            errors.push('Z.AI URL is required when AI services are enabled');
        }
        if (this.config.ai.enabled && !this.config.zai.authentication.api_key) {
            errors.push('Z.AI API key is required when AI services are enabled');
        }
        // Validate performance limits
        if (this.config.performance.resource_limits.max_memory_usage_mb <= 0) {
            errors.push('Max memory usage must be greater than 0');
        }
        if (this.config.performance.resource_limits.max_cpu_usage_percent <= 0 ||
    ...
    exportForMonitoring() {
        return {
            ...
            features_enabled: Object.fromEntries(Object.entries(this.config.ai.features).map(([key, value]) => [key, value.enabled])),
