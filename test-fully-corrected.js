...
/**
 * Fully corrected test with proper enum values and field types for all 16 knowledge types
 */

import { MemoryStoreRequestSchema } from './dist/schemas/enhanced-validation.js';

async function testFullyCorrectedType(typeName, testData) {
  console.log(`\n🧪 Testing ${typeName}...`);
  try {
    const item = {
      kind: typeName,
...
async function runFullyCorrectedTests() {
  console.log('🚀 Fully corrected test with proper enum values and field types...\n');
  ...
  const testCases = [
    ...
    // Final 4 types with corrected enum values and field types
    {
      type: 'incident',
      data: {
        title: 'Test Incident',
    ...
    },
    {
      ...
      data: {
        ...
        category: 'technical',
        validation_status: 'assumed', // Correct enum value
    ...
    },
    {
      type: 'change',
      data: {
        change_type: 'feature_add', // Correct enum value
  ...
  for (const testCase of testCases) {
    const success = await testFullyCorrectedType(testCase.type, testCase.data);
  ...
  console.log('\n' + '='.repeat(60));
  console.log(`📊 FULLY CORRECTED VALIDATION RESULTS: ${successCount}/${totalCount} knowledge types valid`);
  ...
  if (successCount === totalCount) {
    ...
    console.log('🔧 All validation schemas are now working correctly');
  } else {
...
// Run the fully corrected validation tests
runFullyCorrectedTests()
  .then(success => {
