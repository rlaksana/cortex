...
window.CodeMirror = (function () {
  ...
  function CodeMirror(place, options) {
    ...
    this.view = makeView(
      new BranchChunk([new LeafChunk([makeLine('', null, textHeight(display))])])
    );
    this.nextOpId = 0;
    loadMode(this);
    themeChanged(this);
  ...
  function updateDisplayInner(cm, changes, viewPort) {
    ...
    if (intactLines == to - from && from == display.showingFrom && to == display.showingTo) return;
    ...
    for (var node = display.lineDiv.firstChild, height; node; node = node.nextSibling)
      if (node.lineObj) {
        ...
        if (diff > 0.001 || diff < -0.001) updateLineHeight(node.lineObj, height);
      }
  ...
  function buildLineElement(cm, line, lineNo, dims) {
    ...
    if (cm.options.lineNumbers || markers) {
      var gutterWrap = wrap.appendChild(
        elt('div', null, null, 'position: absolute; left: ' + dims.fixedPos + 'px')
      );
    ...
    if (line.widgets)
      for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
        ...
        if (widget.above)
          wrap.insertBefore(
            node,
            cm.options.lineNumbers && line.height != 0 ? gutterWrap : lineElement
          );
  ...
  function updateSelectionRange(cm) {
    ...
    if (sel.from.line == sel.to.line) {
      ...
      if (singleLine) {
        for (var i = 0; i < path.length; i += 3) drawForLine(path[i], path[i + 1], path[i + 2]);
      } else {
        var middleTop,
          middleBot,
        ...
        if (!sel.to.ch) middleBot = heightAtLine(cm, toObj) - display.viewOffset;
        else
  ...
  function intoCoordSystem(cm, lineObj, rect, context) {
    ...
    if (context == 'page') {
      ...
      yOff +=
        lOff.top + (window.pageYOffset || (document.documentElement || document.body).scrollTop);
      var xOff =
        lOff.left + (window.pageXOffset || (document.documentElement || document.body).scrollLeft);
      rect.left += xOff;
      rect.right += xOff;
    }
    rect.top += yOff;
  ...
  function coordsChar(cm, x, y) {
    ...
    for (;;) {
      ...
      if (merged && found.ch == lineRight(lineObj)) lineNo = merged.find().to.line;
      else return found;
    }
  }
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    function getX(ch) {
      var sp = cursorCoords(cm, { line: lineNo, ch: ch }, 'line', lineObj, measurement);
      wrongLine = true;
  ...
  function readInput(cm) {
    ...
    if (same < prevInput.length)
      ...
      to = {
        line: to.line,
        ch: Math.min(getLine(cm.view.doc, to.line).text.length, to.ch + (text.length - same)),
      };
    ...
    function doSelect(cur) {
      ...
      if (type == 'double') {
        ...
        if (posLess(cur, startstart))
          extendSelection(cm, startend, clipPos(doc, { line: cur.line, ch: 0 }));
  ...
  function onDragStart(cm, e) {
    ...
    if (e.dataTransfer.setDragImage && !safari) e.dataTransfer.setDragImage(elt('img'), 0, 0);
  }
    ...
    if (wheelSamples < 20) {
      if (wheelStartX == null) {
        ...
        setTimeout(function () {
          ...
          var sample =
            (movedY && wheelDY && movedY / wheelDY) || (movedX && wheelDX && movedX / wheelDX);
    ...
    if (from.ch == 0 && to.ch == 0 && hlText(lastHL) == '') {
      ...
      if (lines.length == 1) {
        updateLine(
          cm,
          firstLine,
          firstLine.text.slice(0, from.ch) + hlText(lines[0]) + firstLine.text.slice(to.ch),
          hlSpans(lines[0])
        );
      } else {
      ...
      updateLine(
        cm,
        firstLine,
        firstLine.text.slice(0, from.ch) + hlText(lines[0]) + lastLine.text.slice(to.ch),
        hlSpans(lines[0])
      );
  ...
  CodeMirror.prototype = {
    ...
    moveV: operation(null, function (dir, unit) {
      ...
      if (unit == 'page')
        display.scrollbarV.scrollTop += charCoords(this, target, 'div').top - pos.top;
  ...
  CodeMirror.extendMode = function (mode, properties) {
    var exts = modeExtensions.hasOwnProperty(mode)
      ? modeExtensions[mode]
      : (modeExtensions[mode] = {});
    for (var prop in properties) if (properties.hasOwnProperty(prop)) exts[prop] = properties[prop];
  };
  ...
  function markText(cm, from, to, options, type) {
    ...
    if (options)
      for (var opt in options) if (options.hasOwnProperty(opt)) marker[opt] = options[opt];
    ...
    doc.iter(curLine, to.line + 1, function (line) {
      ...
      if (marker.collapsed && curLine == from.line && lineIsHidden(line)) updateLineHeight(line, 0);
      ++curLine;
    });

    if (marker.readOnly) {
      sawReadOnlySpans = true;
      if (cm.view.history.done.length || cm.view.history.undone.length) cm.clearHistory();
    }
    if (marker.collapsed) {
    ...
    if (old)
      for (var i = 0, nw; i < old.length; ++i) {
        var span = old[i],
          marker = span.marker;
        var startsBefore =
          span.from == null || (marker.inclusiveLeft ? span.from <= startCh : span.from < startCh);
  ...
  function markedSpansAfter(old, startCh, endCh) {
    if (old)
      for (var i = 0, nw; i < old.length; ++i) {
        ...
        if (
          endsAfter ||
          (marker.type == 'bookmark' && span.from == endCh && span.from != startCh)
        ) {
          var startsBefore =
            span.from == null || (marker.inclusiveLeft ? span.from <= endCh : span.from < endCh);
          (nw || (nw = [])).push({
  ...
  function lineIsHidden(line) {
    ...
    if (sps)
      for (var sp, i = 0; i < sps.length; ++i) {
        ...
        if (sp.from == 0 && sp.marker.inclusiveLeft && lineIsHiddenInner(line, sp)) return true;
      }
  }
  window.lineIsHidden = lineIsHidden;
  function lineIsHiddenInner(line, span) {
    if (span.to == null || (span.marker.inclusiveRight && span.to == line.text.length)) return true;
  ...
  function styleToClass(style) {
    ...
    return (
      styleToClassCache[style] || (styleToClassCache[style] = 'cm-' + style.replace(/ +/g, ' cm-'))
    );
  }
      ...
      if (nextChange == pos) {
        ...
        for (var j = 0; j < spans.length; ++j) {
          ...
          if (sp.from <= pos && (sp.to == null || sp.to > pos)) {
            ...
            if (m.collapsed && (!collapsed || collapsed.marker.width < m.width)) collapsed = sp;
    ...
    while (target != null) {
      if (part.level % 2 == linedir) {
        if (target < part.from || target > part.to) {
          ...
          target =
            part &&
            (dir > 0 == part.level % 2 ? moveOneUnit(part.to, -1) : moveOneUnit(part.from, 1));
        } else break;
      } else {
    ...
    return function charOrdering(str) {
      ...
      for (var i = 0; i < len; ++i) {
        ...
        if (type == ',') types[i] = 'N';
          ...
          var replace =
            (i && types[i - 1] == '!') || (end < len - 1 && types[end] == '1') ? '1' : 'N';
