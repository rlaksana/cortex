...
-- ============================================================================
-- MANDATORY: This schema requires PostgreSQL 18. DO NOT use with PostgreSQL 16 or other versions.
-- - Uses gen_random_uuid() from pgcrypto extension (NOT uuid-ossp)
-- - PostgreSQL 18 specific features and performance optimizations
-- - All UUID generation uses gen_random_uuid() for better security and performance
-- ============================================================================
-- Migration ID: 001
-- Description: Complete schema for all 16 knowledge types with audit trail
-- ============================================================================

-- This migration creates the complete database schema for the Cortex Memory MCP system.
-- It includes all core tables, indexes, triggers, and functions needed for production use.

-- Execute with: psql -d cortex_prod -f migrations/001_complete_schema.sql
...
-- ============================================================================
-- AUTO-PURGE METADATA TABLE
-- ============================================================================

-- Metadata for automatic cleanup operations
CREATE TABLE IF NOT EXISTS purge_metadata (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT purge_metadata_singleton_check CHECK (id = 1)
);

-- Create initial purge metadata record
INSERT INTO purge_metadata (id)
VALUES (1)
ON CONFLICT (id) DO NOTHING;

-- Create index for efficient queries
CREATE INDEX IF NOT EXISTS idx_purge_metadata_enabled ON purge_metadata(enabled) WHERE enabled = TRUE;

-- Add trigger to update updated_at column
CREATE OR REPLACE FUNCTION update_purge_metadata_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE TRIGGER purge_metadata_updated_at
  BEFORE UPDATE ON purge_metadata
  FOR EACH ROW
  EXECUTE FUNCTION update_purge_metadata_updated_at();

-- ============================================================================
-- MIGRATION COMPLETE
-- ============================================================================

-- Record this migration in ddl_history
INSERT INTO ddl_history (migration_id, ddl_text, checksum, description, status)
VALUES (
  '001_complete_schema',
  'Complete database schema for Cortex Memory MCP system',
  'sha256_placeholder', -- This will be updated by the migration system
  'Initial schema creation with all 16 knowledge types',
  'applied'
);

-- Migration summary
-- ✓ 20 Core tables: 16 knowledge types + 3 system tables (event_audit, ddl_history, purge_metadata)
-- ✓ Comprehensive indexing strategy for performance
-- ✓ Automatic audit trail with triggers
-- ✓ Full-text search capabilities
-- ✓ Graph relationship support
-- ✓ Auto-purge functionality with metadata tracking
-- ✓ 8-LOG SYSTEM for session persistence
-- ✓ Generated columns and JSONB optimization
-- ✓ Views for common query patterns
-- ✓ Table statistics monitoring function
