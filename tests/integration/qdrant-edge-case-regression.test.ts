...
describe('Qdrant Edge Case Regression Suite', () => {
  ...
  describe('Data Consistency Edge Cases', () => {
    it('should handle malformed vector embeddings', async () => {
    ...
    it('should handle duplicate item IDs with different content', async () => {
  ...
  describe('Search Performance Edge Cases', () => {
    it('should handle empty and null queries gracefully', async () => {
      const testQueries = [
        '',
        '   ',
        null as any,
  ...
  describe('Hybrid Search Edge Cases', () => {
    it('should handle hybrid search with partial failures', async () => {
      // Mock semantic search failure
      const originalHybridSearch = qdrantAdapter.hybridSearch;
      let callCount = 0;

      qdrantAdapter.hybridSearch = async function(query: string, options: HybridSearchOptions = {}) {
        callCount++;
        if (callCount === 1) {
    ...
    it('should maintain deterministic ordering across multiple calls', async () => {
    ...
    it('should handle concurrent hybrid search requests', async () => {
      const concurrentQueries = 20;
      const queries = Array.from({ length: concurrentQueries }, (_, i) =>
        `concurrent hybrid search query ${i}`
      );

      const promises = queries.map(query =>
        qdrantAdapter.hybridSearch(query, {
          deterministic: true,
          limit: 5,
          enableFallback: true,
  ...
  describe('Circuit Breaker Integration', () => {
    ...
    it('should log circuit breaker events appropriately', async () => {
      const logEvents: any[] = [];
    ...
    it('should handle concurrent operations without deadlocks', async () => {
      ...
      for (let i = 0; i < concurrentOperations; i++) {
        if (i % 3 === 0) {
          ...
          operations.push(
            qdrantAdapter.hybridSearch(`concurrent hybrid search ${i}`, {
              limit: 5,
              deterministic: true,
