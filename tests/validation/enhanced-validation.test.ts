...
import { validateKnowledgeItems, MemoryStoreRequestSchema } from '../../src/schemas/enhanced-validation.js';
...
describe('Enhanced Validation Tests', () => {
  describe('Section Validation', () => {
    ...
    it('should require scope for section items', () => {
      ...
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0].message).toContain('Section items must have a scope');
    });
  });
  ...
  describe('Decision Validation', () => {
    it('should reject insufficient rationale for accepted decisions', () => {
      ...
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0].message).toContain('detailed rationale');
    });

    it('should accept sufficient rationale for accepted decisions', () => {
      const sufficientRationale = 'This decision was made because security is critical for our application and we need to ensure proper authentication mechanisms are in place to protect user data.';
      const result = validateKnowledgeItems([{
        kind: 'decision',
        data: {
          component: 'auth',
  ...
  describe('Edge Cases from Testing', () => {
    it('should handle whitespace-only titles', () => {
      ...
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0].message).toContain('cannot start with whitespace');
    });
    ...
    it('should require at least one content field', () => {
      ...
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0].message).toContain('must have at least one');
    });
  });
  ...
  describe('Autonomous Context Testing', () => {
    it('should provide proper autonomous context for validation failures', async () => {
      ...
      expect(result.autonomous_context.user_message_suggestion).toContain('validation errors found');
    });
