          ...
          const result = scenario.timeout
            ? await Promise.race([
                ...
                new Promise((_, reject) =>
                  setTimeout(
                    () => reject(new Error(`Test timeout after ${scenario.timeout}ms`)),
                    scenario.timeout
                  )
                ),
              ])
      ...
      for (const scenario of PERFORMANCE_SCENARIOS.memory_find) {
        it(
          ...
          async () => {
            ...
            const duration = endTime - startTime;
            console.log(
              `${scenario.name}: ${duration}ms for ${scenario.queryCount} searches over ${scenario.itemCount} items`
            );
