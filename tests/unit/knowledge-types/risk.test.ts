...
import { RiskSchema, validateKnowledgeItem } from '../../../src/schemas/knowledge-types';
...
describe('Risk Knowledge Type - Comprehensive Testing', () => {
    ...
    it('should validate complete risk with all fields', () => {
      const risk = {
        ...
        data: {
          ...
          impact_description:
            'Authentication bypass could allow unauthorized access to all user data, leading to data breach and privacy violations. Estimated impact includes regulatory fines, customer loss, and brand damage.',
          trigger_events: [
          ...
          contingency_plans:
            'If breach occurs, immediately activate incident response team, notify affected users within 72 hours, and engage external security forensics team.',
        },
    ...
    it('should store critical security risk successfully', async () => {
      const risk = {
        ...
        data: {
          ...
          impact_description:
            'Exposure of database encryption keys would compromise all stored data, including user credentials and sensitive business information.',
          status: 'active' as const,
      ...
      const risks = [
        {
          ...
          data: {
            ...
            impact_description:
              'Payment processing service failure could prevent revenue generation',
            status: 'active' as const,
    ...
    it('should store risk with comprehensive contingency plans', async () => {
      const risk = {
        ...
        data: {
          ...
          probability: 'unlikely' as const,
          impact_description:
            'Extended data center outage could result in complete service unavailability and data loss if backup systems fail.',
          status: 'active' as const,
  ...
  describe('Risk Scope Isolation', () => {
    it('should isolate risks by project scope', async () => {
      ...
      const results = await Promise.all(risks.map((risk) => db.storeItems([risk])));
    ...
    it('should handle different branch scopes within same project', async () => {
      ...
      await Promise.all(risks.map((risk) => db.storeItems([risk])));
  ...
  describe('Risk Edge Cases and Error Handling', () => {
    ...
    it('should handle risk with special characters in title and description', async () => {
      const risk = {
        ...
        data: {
          title:
            '🚨 Critical: API endpoint /api/v2/users/{id} returns 500 error with SQL injection vulnerability (CVE-2025-1234)',
          category: 'security' as const,
          risk_level: 'critical' as const,
          probability: 'very_likely' as const,
          impact_description:
            'The API endpoint is vulnerable to SQL injection attacks when user ID contains special characters like @, #, $, %. This could allow attackers to dump the entire database and access sensitive user information including passwords and payment data.',
          status: 'active' as const,
    ...
    it('should handle risk with long impact description within limits', async () => {
      const longImpactDescription =
        'This risk represents a significant threat to our business operations and could result in substantial financial losses, regulatory penalties, reputational damage, customer churn, legal liabilities, operational disruptions, competitive disadvantages, and strategic setbacks. '.repeat(
          5
        );
    ...
    it('should handle risk with multiple related decisions', async () => {
      const risk = {
        ...
        data: {
          ...
          probability: 'possible' as const,
          impact_description:
            'Migration to new technology stack may introduce bugs and performance issues',
          status: 'active' as const,
    ...
    it('should handle risk acceptance with justification', () => {
      const risk = {
        ...
        data: {
          ...
          probability: 'very_likely' as const,
          impact_description:
            'Some legacy components have minor UI inconsistencies that do not affect functionality',
          status: 'accepted' as const,
          owner: 'product-manager',
          mitigation_strategies: [
            'Accept as-is due to low priority',
            'Address during future redesign',
          ],
        },
      };
    ...
    it('should handle risk with complex trigger events', async () => {
      const risk = {
        ...
        data: {
          ...
          probability: 'possible' as const,
          impact_description:
            'Supply chain disruptions could delay product delivery and increase costs',
          status: 'active' as const,
          owner: 'operations-team',
          trigger_events: [
    ...
    it('should handle risk with probability and impact quantification', () => {
      const risk = {
        ...
        data: {
          ...
          probability: 'unlikely' as const,
          impact_description:
            'Data breach could affect up to 10 million customers, resulting in estimated costs of $50M including fines, legal fees, and customer compensation. Regulatory penalties could be up to $25M under GDPR and CCPA.',
          status: 'active' as const,
          owner: 'ciso',
    ...
    it('should integrate with knowledge item validation', () => {
      const risk = {
        ...
        data: {
          ...
          probability: 'unlikely' as const,
          impact_description:
            'Cloud provider SLA violations could result in service unavailability and financial credits that may not cover business losses',
          status: 'active' as const,
          owner: 'infrastructure-team',
          mitigation_strategies: [
    ...
    it('should handle risk with comprehensive metadata', () => {
      const risk = {
        ...
        data: {
          ...
          probability: 'possible' as const,
          impact_description:
            'AI models may exhibit bias leading to unfair outcomes and potential regulatory compliance issues',
          status: 'active' as const,
    ...
    it('should handle risk closure with lessons learned', async () => {
      const risk = {
        ...
        data: {
          ...
          contingency_plans:
            'Risk successfully mitigated through system modernization and redundancy implementation. Lessons learned: prioritize technical debt reduction, invest in monitoring, and establish clear migration timelines.',
