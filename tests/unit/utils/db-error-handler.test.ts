/**
 * Database Error Handler Comprehensive Test Suite
 *
...
describe('DatabaseErrorHandler', () => {
  ...
  describe('Logging', () => {
    ...
    it('should log operation failures', async () => {
      ...
      expect(mockedLogger.warn).toHaveBeenCalledTimes(2); // Initial failure + retry failure
...
describe('safeDbOperation Helper', () => {
  ...
  beforeEach(() => {
    ...
    (dbErrorHandler as any) = mockErrorHandler;
  });

  afterEach(() => {
    // Restore original singleton
    (dbErrorHandler as any) = new DatabaseErrorHandler();
  });
  ...
  it('should return data when operation succeeds', async () => {
    ...
    const result = await safeDbOperation(mockOperation, 'test-operation');
  ...
  it('should throw error when operation fails', async () => {
    ...
    await expect(safeDbOperation(mockOperation, 'test-operation'))
