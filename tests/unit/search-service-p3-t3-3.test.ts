...
// Test data factories
const createMockSearchResult = (id: string, kind: string, matchType: string, data: any = {}): any => ({
  id,
  kind,
  data: {
    title: `Test ${kind} ${id}`,
    ...
    it('should handle mixed quality results with appropriate match_types', async () => {
      ...
      if (result.results.length > 1) {
        for (let i = 0; i < result.results.length - 1; i++) {
          expect(result.results[i].confidence_score)
            .toBeGreaterThanOrEqual(result.results[i + 1].confidence_score);
        }
      }
    });
