  ...
  describe('Hybrid Fallback Strategy Implementation', () => {
    describe('Semantic + Sparse Hybrid Search', () => {
      it('should implement hybrid fallback with semantic and sparse search', async () => {
        ...
        const hybridFallbackLogs = auditLogs.filter(log =>
          log.event_type === 'hybrid_fallback' && log.details.fallback_type === 'semantic_to_sparse'
        );
      ...
      it('should switch between semantic and sparse based on performance', async () => {
        ...
        vi.spyOn(searchManager as any, 'performSemanticSearch').mockImplementation(async () => {
          ...
          return [
            { id: 'semantic-1', content: 'semantic result', score: semanticPerformance },
          ];
        });

        vi.spyOn(searchManager as any, 'performSparseSearch').mockImplementation(async () => {
          return [
            { id: 'sparse-1', content: 'sparse result', score: sparsePerformance },
          ];
        });
      ...
      it('should maintain result quality during hybrid fallback transitions', async () => {
        ...
        const hybridAvgScore = hybridResult.results.reduce((sum, r) => sum + r.score, 0) / hybridResult.results.length;
    ...
    describe('Intelligent Strategy Thresholds', () => {
      it('should adapt thresholds based on historical performance', async () => {
        ...
        const avgLaterPerformance = laterResults.reduce((sum, r) => sum + r.confidence, 0) / laterResults.length;
      ...
      it('should implement context-aware timeout management', async () => {
        ...
        for (const config of contextConfigs) {
          vi.spyOn(searchManager as any, 'getContextualTimeout').mockReturnValue(config.expectedTimeout);
  ...
  describe('Confidence Calibration System', () => {
    describe('Dynamic Confidence Adjustment', () => {
      it('should calibrate confidence based on actual search performance', async () => {
        ...
        vi.spyOn(searchManager as any, 'performVectorSearch').mockImplementation(async () => {
          ...
          return [
            { id: `calibrated-${performanceIndex}`, content: 'calibrated result', score: actualScore },
          ];
        });

        vi.spyOn(searchManager as any, 'calculateCalibratedConfidence').mockImplementation((results) => {
          const avgScore = results.reduce((sum, r) => sum + r.score, 0) / results.length;
          const recentPerformances = actualPerformanceScores.slice(-3);
          const avgRecentPerformance = recentPerformances.reduce((a, b) => a + b, 0) / recentPerformances.length;
      ...
      it('should adjust confidence thresholds for different search strategies', async () => {
        ...
        for (const config of strategyConfigs) {
          vi.spyOn(searchManager as any, 'getStrategyConfidenceThreshold').mockReturnValue(config.expectedConfidence);

          vi.spyOn(searchManager as any, 'performVectorSearch').mockResolvedValue([
            { id: `${config.strategy}-result`, content: 'strategy result', score: config.expectedConfidence },
          ]);
      ...
      it('should implement confidence-based result filtering', async () => {
        ...
        vi.spyOn(searchManager as any, 'performVectorSearch').mockResolvedValue(mixedQualityResults);

        vi.spyOn(searchManager as any, 'filterResultsByConfidence').mockImplementation((results, threshold) => {
    ...
    describe('Performance Regression Detection', () => {
      it('should detect performance regression within 1% threshold', async () => {
        ...
        vi.spyOn(searchManager as any, 'detectPerformanceRegression').mockImplementation((baseline, current) => {
        ...
        // Detect regression
        const regressionAnalysis = searchManager['detectPerformanceRegression'](baselinePerformance, result.confidence);
        ...
        expect(result.metadata.performanceRegression.percentage).toBe(regressionAnalysis.regressionPercentage);
      });
  ...
  describe('Load Testing with Quality Metrics', () => {
    describe('High-Load Scenarios', () => {
      ...
      it('should implement intelligent load shedding when quality drops', async () => {
        ...
        const loadSheddingResults = successfulResults.filter(r =>
          r.metadata.loadShedding?.activated
        );
    ...
    describe('Performance Regression Prevention', () => {
      it('should prevent performance regression through adaptive optimization', async () => {
        ...
        vi.spyOn(searchManager as any, 'shouldOptimizePerformance').mockImplementation((currentPerformance) => {
          const regressionPercentage = ((baselinePerformance - currentPerformance) / baselinePerformance) * 100;
        ...
        const laterAvgPerformance = laterResults.reduce((sum, r) => sum + r.confidence, 0) / laterResults.length;
