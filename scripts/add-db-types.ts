 ...
 *
 * Creates comprehensive type definitions for all database tables
 * and updates pool.query() calls to use proper generics.
 *
 * This fixes ~90% of lint errors related to unsafe any access.
 */
...
export interface ChangeLogRow {
  id: string;
  change_type: 'feature_add' | 'feature_modify' | 'feature_remove' | 'bugfix' | 'refactor' | 'config_change' | 'dependency_update';
  subject_ref: string;
  summary: string;
  details: string | null;
  content_hash: string;
...
async function main() {
  console.log('Creating database type definitions...\n');

  // Create types file
  const typesPath = join(process.cwd(), 'src', 'types', 'db-rows.ts');
  writeFileSync(typesPath, typeDefinitions, 'utf8');
  console.log('✅ Created:', typesPath);

  console.log('\nNext steps:');
  console.log('1. Import types in files with errors:');
  console.log('   import type { SectionRow, ADRDecisionRow } from "../types/db-rows.js";');
  console.log('');
  console.log('2. Add type generics to queries:');
  console.log('   pool.query<SectionRow>("SELECT * FROM section...")');
  console.log('');
  console.log('3. Run: npm run build && npm run lint');
}
