  ...
  .then(({ memoryStore }) => {
    ...
    const releaseNoteTest = {
      ...
      data: {
        ...
        new_features:
          'Knowledge storage system, Full-text search capabilities, Memory validation framework',
        bug_fixes:
          'Fixed connection pooling issues, Resolved validation errors, Corrected field mapping problems',
      },
    };

    console.log('🔧 Testing Release Note with corrected array field format...');

    memoryStore([releaseNoteTest])
      .then((result) => {
