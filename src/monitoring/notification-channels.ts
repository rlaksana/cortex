...
/**
 * Type guard to check if alert has required properties
 */
const isAlertWithSource = (alert: unknown): alert is Alert & {
  source: { component: string; metric: string; value: unknown; threshold: unknown };
} => {
  return (
    alert &&
    typeof alert === 'object' &&
    ...
    'component' in alert.source &&
    'metric' in alert.source &&
    typeof alert.source.component === 'string' &&
    typeof alert.source.metric === 'string'
  );
};

/**
 * Safe getter for alert properties
 */
const safeGetAlertProperty = <T>(
  alert: Alert | unknown,
  property: string,
  defaultValue: T
): T => {
  ...
  const value = (alert as Record<string, unknown>)[property];
...
export class EmailNotificationChannel implements NotificationChannel {
  ...
  private generateEmailContent(alert: Alert, config: EmailConfig): EmailContent {
  ...
  private processTemplate(template: string, alert: Alert, config: EmailConfig): string {
    // Safely access alert properties with fallbacks
    const alertId = safeGetAlertProperty(alert, 'id', 'unknown');
    const alertTitle = safeGetAlertProperty(alert, 'title', 'Unknown Alert');
    const alertMessage = safeGetAlertProperty(alert, 'message', 'No message');
    const alertSeverity = String(safeGetAlertProperty(alert, 'severity', 'unknown')).toUpperCase();

    let sourceComponent = 'unknown';
    let sourceMetric = 'unknown';
    let sourceValue = 'unknown';
    let sourceThreshold = 'unknown';

    if (isAlertWithSource(alert) && alert.source) {
      sourceComponent = alert.source.component;
      sourceMetric = alert.source.metric;
      sourceValue = String(alert.source.value || 'unknown');
      sourceThreshold = String(alert.source.threshold || 'unknown');
    }

    const alertTimestamp = alert.timestamp instanceof Date ? alert.timestamp.toISOString() : new Date().toISOString();
    const runbookUrl = this.generateRunbookUrl(alert, config);

    return template
      .replace(/\{\{alert\.id\}\}/g, alertId)
      .replace(/\{\{alert\.title\}\}/g, alertTitle)
      .replace(/\{\{alert\.message\}\}/g, alertMessage)
      .replace(/\{\{alert\.severity\}\}/g, alertSeverity)
      .replace(/\{\{alert\.source\.component\}\}/g, sourceComponent)
      .replace(/\{\{alert\.timestamp\}\}/g, alertTimestamp)
      .replace(/\{\{alert\.runbookUrl\}\}/g, runbookUrl);
  }

  private getEmailTemplate(templateName: string, format: 'html' | 'text'): string {
...
<body>
    <div class="alert-critical">
        <h2>🚨 CRITICAL: Database Connectivity Loss</h2>
...
<body>
    <div class="alert-warning">
        <h2>⚠️ WARNING: High Memory Usage Detected</h2>
...
`,
  ...
  private generateRunbookUrl(alert: Alert, config: EmailConfig): string {
...
export class SlackNotificationChannel implements NotificationChannel {
  ...
  private generateSlackPayload(alert: Alert, config: SlackConfig): SlackPayload {
    // Safely access alert properties with fallbacks
    const alertSeverity = String(safeGetAlertProperty(alert, 'severity', 'unknown'));
    const alertTitle = safeGetAlertProperty(alert, 'title', 'Unknown Alert');
    const alertMessage = safeGetAlertProperty(alert, 'message', 'No message');
    const alertStatus = safeGetAlertProperty(alert, 'status', 'unknown');
    const alertTimestamp = alert.timestamp instanceof Date ? alert.timestamp : new Date();

    const color = this.getSeverityColor(alertSeverity);
    const emoji = this.getSeverityEmoji(alertSeverity);
      ...
      attachments: [
        {
          ...
          fields: [
            {
              title: 'Severity',
              value: alertSeverity.toUpperCase(),
              short: true,
            },
            {
              title: 'Component',
            ...
            {
              title: 'Metric',
              value: `${sourceMetric}: ${sourceValue}`,
              short: true,
            },
            {
              title: 'Threshold',
            ...
            {
              title: 'Time',
              value: alertTimestamp.toLocaleString(),
              short: true,
            },
            {
              title: 'Status',
          ...
          ts: Math.floor(alertTimestamp.getTime() / 1000),
  ...
  private generateSlackActions(alert: Alert, config: SlackConfig): SlackAction[] {
...
export class PagerDutyNotificationChannel implements NotificationChannel {
  ...
  private generatePagerDutyEvent(alert: Alert, config: PagerDutyConfig): PagerDutyEvent {
    // Safely access alert properties with fallbacks
    const alertSeverity = String(safeGetAlertProperty(alert, 'severity', 'unknown'));
    const alertTitle = safeGetAlertProperty(alert, 'title', 'Unknown Alert');
    const alertMessage = safeGetAlertProperty(alert, 'message', 'No message');
    const alertStatus = safeGetAlertProperty(alert, 'status', 'unknown');
    const alertId = safeGetAlertProperty(alert, 'id', 'unknown');
    const alertRuleId = safeGetAlertProperty(alert, 'ruleId', 'unknown');
    const alertRuleName = safeGetAlertProperty(alert, 'ruleName', 'Unknown Rule');
    const alertTimestamp = alert.timestamp instanceof Date ? alert.timestamp : new Date();
    ...
    const metadata = safeGetAlertProperty(alert, 'metadata', {});
    const runbookId = metadata && typeof metadata === 'object' && 'runbookId' in metadata
      ? String(metadata.runbookId)
      : undefined;

    const severity = this.mapSeverityToPagerDuty(alertSeverity);
...
export class TeamsNotificationChannel implements NotificationChannel {
  ...
  private generateTeamsPayload(alert: Alert, config: TeamsConfig): TeamsPayload {
  ...
  private generateTeamsActions(alert: Alert, config: TeamsConfig): TeamsAction[] {
...
export class WebhookNotificationChannel implements NotificationChannel {
  ...
  private generateWebhookPayload(alert: Alert, config: WebhookConfig): unknown {
    ...
    const alertTitle = safeGetAlertProperty(alert, 'title', 'Unknown Alert');
    const alertMessage = safeGetAlertProperty(alert, 'message', 'No message');
    const alertSeverity = safeGetAlertProperty(alert, 'severity', 'unknown');
    const alertStatus = safeGetAlertProperty(alert, 'status', 'unknown');
    const alertRuleId = safeGetAlertProperty(alert, 'ruleId', 'unknown');
    const alertRuleName = safeGetAlertProperty(alert, 'ruleName', 'Unknown Rule');
    const alertTimestamp = alert.timestamp instanceof Date ? alert.timestamp : new Date();
    const alertMetadata = safeGetAlertProperty(alert, 'metadata', {});
  ...
  private applyTemplate(template: unknown, basePayload: unknown, alert: Alert): unknown {
    ...
    if (typeof template === 'string') {
      const basePayloadRecord = safeObjectAccess(basePayload, {});
      const alertData = safeGetProperty(basePayloadRecord, 'alert', alert);
      return JSON.parse(template.replace(/\{\{alert\}\}/g, JSON.stringify(alertData)));
    } else if (safeIsObject(template)) {
      const basePayloadRecord = safeObjectAccess(basePayload, {});
      return { ...template, ...basePayloadRecord };
    }
    return basePayload;
  }
...
export class SNSNotificationChannel implements NotificationChannel {
  ...
  private generateSNSMessage(alert: Alert, config: SNSConfig): SNSMessage {
    const message = {
      alert: {
        id: alert.id,
        title: alert.title,
