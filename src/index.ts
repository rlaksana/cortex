...
class Logger {
  constructor(private level: string = env.LOG_LEVEL) {}
  ...
  try {
    ...
    switch (name) {
      case 'memory_store':
        ...
        break;
      case 'memory_find':
        result = await handleMemoryFind(args as { query: string; limit?: number; types?: string[]; scope?: any });
        break;
    ...
    const duration = Date.now() - startTime;
    logger.error(`Tool execution failed: ${name} (${duration}ms)`, error instanceof Error ? error.message : String(error));
...
async function handleMemoryFind(args: { query: string; limit?: number; types?: string[]; scope?: any }) {
  if (!args.query) {
...
async function startServer(): Promise<void> {
  try {
    logger.info('Starting Cortex Memory MCP Server...');
    ...
    logger.info('Cortex Memory MCP Server connected and ready!');
