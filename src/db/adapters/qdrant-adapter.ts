...
export class QdrantAdapter implements IVectorAdapter {
  ...
  constructor(config: VectorConfig) {
    ...
    this.capabilities = {
      ...
      supportedOperations: [
        ...
        'similarityDetection', 'deduplication'
      ]
    };
  }
  ...
  async getMetrics(): Promise<DatabaseMetrics> {
    try {
      ...
      return {
        ...
        storageSize: collectionInfo.result?.vectors_count || 0,
  ...
  async delete(ids: string[], options: DeleteOptions = {}): Promise<{ deleted: number; errors: StoreError[] }> {
    ...
    const { cascade = false, soft = false, validate = true } = options;
    let deleted = 0;
    const errors: StoreError[] = [];
    ...
    try {
      ...
      switch (mode) {
        ...
        case 'deep':
          ...
          break;
        case 'fast':
          searchResults = await this.keywordSearch(query.query, options);
          break;
        case 'auto':
        default:
      ...
      const typeFilteredResults = query.types && query.types.length > 0 ?
      ...
      const autonomousContext = {
        ...
        confidence_average: limitedResults.length > 0 ?
          limitedResults.reduce((sum, r) => sum + r.confidence_score, 0) / limitedResults.length : 0,
        user_message_suggestion: limitedResults.length > 0 ?
          `Found ${limitedResults.length} relevant items` : 'No items found matching your query'
      };
  ...
  async semanticSearch(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    try {
      ...
      throw new DatabaseError('Failed to perform semantic search', 'SEMANTIC_SEARCH_ERROR', error as Error);
    }
  }
  ...
  async exactSearch(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    try {
      ...
      throw new DatabaseError('Failed to perform exact search', 'EXACT_SEARCH_ERROR', error as Error);
    }
  }

  async findNearest(embedding: number[], limit: number = 10, threshold: number = 0.5): Promise<SearchResult[]> {
    await this.ensureInitialized();
    ...
    try {
      ...
      throw new DatabaseError('Failed to find nearest vectors', 'FIND_NEAREST_ERROR', error as Error);
    }
  }
  ...
  async hybridSearch(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    const { keyword_weight = 0.3, semantic_weight = 0.7, limit = 50 } = options;

    try {
      // Generate both embeddings
      const [queryEmbedding, querySparse] = await Promise.all([
      ...
      throw new DatabaseError('Failed to perform hybrid search', 'HYBRID_SEARCH_ERROR', error as Error);
    }
  }
  ...
  async findByScope(scope: { project?: string; branch?: string; org?: string }, options: SearchOptions = {}): Promise<KnowledgeItem[]> {
    await this.ensureInitialized();
    ...
    try {
      ...
      const response = await this.client.search(this.COLLECTION_NAME, {
        ...
        limit: options.cache ? 1000 : options.cache, // Large limit for scope search
        score_threshold: 0.1,
        with_payload: true,
        filter: searchFilter
      });
      ...
      throw new DatabaseError('Failed to search by scope in Qdrant', 'SCOPE_SEARCH_ERROR', error as Error);
    }
  }

  // === Advanced Operations ===

  async findSimilar(item: KnowledgeItem, threshold: number = 0.7, options: SearchOptions = {}): Promise<SearchResult[]> {
    await this.ensureInitialized();
    ...
    try {
      ...
      throw new DatabaseError('Failed to find similar items in Qdrant', 'SIMILARITY_SEARCH_ERROR', error as Error);
    }
  }
  ...
  async getStatistics(scope?: { project?: string; branch?: string; org?: string }): Promise<{
    ...
    try {
      // Get collection info
      const collectionInfo = await this.client.getCollection(this.COLLECTION_NAME);
      const totalItems = collectionInfo.result?.vectors_count || 0;
      ...
      for (const point of sample.result.points) {
        const kind = point.payload?.kind || 'unknown';
      ...
      return {
        totalItems,
        itemsByKind,
        storageSize: totalItems * (this.config.vectorSize * 4), // Rough estimate
        lastUpdated
      };
  ...
  async bulkDelete(filter: { kind?: string; scope?: any; before?: string }, options: DeleteOptions = {}): Promise<{ deleted: number }> {
    await this.ensureInitialized();
    ...
    try {
      ...
      if (toDelete.result.points.length === 0) {
        return { deleted: 0 };
      }

      const ids = toDelete.result.points.map(point => point.id);
      ...
      throw new DatabaseError('Failed to bulk delete items in Qdrant', 'BULK_DELETE_ERROR', error as Error);
    }
  }
  ...
  async storeWithEmbeddings(items: Array<KnowledgeItem & { embedding: number[] }>, options: StoreOptions = {}): Promise<MemoryStoreResponse> {
    ...
    try {
      ...
      logger.error({ error, itemCount: items.length }, 'Failed to store items with embeddings in Qdrant');
      throw new DatabaseError('Failed to store items with embeddings', 'STORE_WITH_EMBEDDINGS_ERROR', error as Error);
    }
  }
  ...
  async vectorSearch(embedding: number[], options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    try {
      ...
      throw new DatabaseError('Failed to perform vector search', 'VECTOR_SEARCH_ERROR', error as Error);
    }
  }
  ...
  private searchResultToSearchResult(result: any, matchType: string): SearchResult {
    ...
    return {
      ...
      match_type: matchType as 'exact' | 'fuzzy' | 'semantic' | 'hybrid',
      highlight: payload.content ? [`${payload.content.substring(0, 200)  }...`] : undefined
    };
  }
  ...
  private keywordSearch(query: string, options: SearchOptions = {}): Promise<any[]> {
    // Use sparse vector search for keyword search
    const querySparse = this.generateSparseVector(query);
    const { limit = 50, score_threshold = 0.3 } = options;
  ...
  private generateAutonomousContext(stored: StoreResult[], errors: StoreError[]): AutonomousContext {
    ...
    return {
      ...
      recommendation: duplicatesFound > 0 ?
        'Review duplicates before storing' :
  ...
  async getCollectionStats(): Promise<{
    ...
    try {
      ...
      throw new DatabaseError('Failed to retrieve collection stats', 'COLLECTION_STATS_ERROR', error as Error);
    }
  }
  ...
  async getCollectionInfo(): Promise<any> {
    try {
      ...
      throw new DatabaseError('Failed to retrieve collection info', 'COLLECTION_INFO_ERROR', error as Error);
    }
  }
}