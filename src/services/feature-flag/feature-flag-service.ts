...
export class FeatureFlagService extends EventEmitter {
   ...
   */
  emergencyDisableFlag(id: string, reason?: string): boolean {
    const flag = this.flags.get(id);
    if (!flag) {
      return false;
    }
   ...
   */
  createCohort(cohort: Omit<UserCohort, 'id' | 'createdAt' | 'updatedAt'>): UserCohort {
    const id = this.generateId();
    const now = new Date();
   ...
   * Get a cohort by ID
   */
  getCohort(id: string): UserCohort | undefined {
   ...
   */
  private evaluateSingleCondition(condition: FlagCondition, userContext: UserContext): boolean {
   ...
   */
  private evaluateCohortCriteria(cohort: UserCohort, userContext: UserContext): boolean {
   ...
   */
  private hashUserForPercentage(userId: string, seed: string): number {
   ...
   * Generate cache key
   */
  private generateCacheKey(flagId: string, userContext: UserContext): string {
   ...
   * Get from cache
   */
  private getFromCache(key: string): FlagEvaluationResult | null {
   ...
   * Set cache
   */
  private setCache(key: string, result: FlagEvaluationResult): void {
