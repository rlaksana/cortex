...
export class CanaryOrchestrator extends EventEmitter {
  ...
  createDeployment(
    ...
    // Initialize phases
    deploymentConfig.phases = deploymentConfig.phases.map((phase, index) => ({
      ...phase,
   ...
   */
  async pauseDeployment(deploymentId: string, reason?: string): Promise<boolean> {
  ...
  async rollbackDeployment(
    ...
    const rollbackEvent: RollbackEvent = {
      ...
      trafficPercentage: deployment.currentTrafficPercentage,
      metrics: this.getCurrentPhaseMetrics(deployment),
      triggeredBy,
    };
   ...
   */
  private calculateTrafficPercentage(deployment: CanaryDeployment, phaseIndex: number): number {
   ...
   */
  private stopDeploymentMonitoring(deploymentId: string): void {
   ...
   */
  getTrafficRouting(deploymentId: string): TrafficRouting | undefined {
