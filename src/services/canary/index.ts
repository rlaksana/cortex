...
export const CanaryUtils = {
   ...
   */
  createSimpleCanaryConfig: (overrides: Partial<CanaryDeploymentConfig> = {}): CanaryDeploymentConfig => {
    const now = new Date();
    return {
   ...
   */
  calculateTrafficProgression: (phases: number, strategy: TrafficShiftStrategy = TrafficShiftStrategy.LINEAR): number[] => {
  ...
  generateRecommendations: (config: CanaryDeploymentConfig): string[] => {
    ...
    if (config.initialTrafficPercentage > 20) {
      recommendations.push('Consider starting with lower initial traffic (5-10%) for safer rollouts');
    }
    ...
    if (config.maxDeploymentTimeMs > 7200000) { // 2 hours
