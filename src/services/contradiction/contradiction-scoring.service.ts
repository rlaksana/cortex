...
export class ContradictionScoringService {
   ...
   */
  private assessUrgency(contradiction: ContradictionResult, items: KnowledgeItem[]): number {
  ...
  private async assessBusinessValue(
    ...
    const indicatorCount = businessIndicators.reduce((count, indicator) => {
   ...
   * Count dependencies
   */
  private countDependencies(contradiction: ContradictionResult, items: KnowledgeItem[]): number {
  ...
  private assessStakeholderImpact(
    ...
    const indicatorCount = stakeholderIndicators.reduce((count, indicator) => {
      return count + (content.includes(indicator) ? 1 : 0);
    }, 0);
   ...
   */
  private assessRiskExposure(contradiction: ContradictionResult, items: KnowledgeItem[]): number {
...
${JSON.stringify(baseFactors, null, 2)}
...
Respond with a JSON object containing these adjusted scores and your reasoning:
{
  ...
  private generateScoreExplanation(
    factors: ScoringFactors,
    weightedScores: ScoringFactors['weighted_scores'],
    totalScore: number
  ): string {
    ...
    return (
      explanations.join('. ') ||
      `Score of ${totalScore.toFixed(3)} based on weighted factor analysis`
    );
  }
  ...
  > {
    ...
    if (typeDistribution.semantic > typeDistribution.logical * 2) {
      insights.push({
        ...
        description: 'High number of semantic contradictions detected',
        affected_contradictions: scoredContradictions
    ...
    if (severityDistribution.critical > 0) {
      insights.push({
        type: 'severity_alert',
        description: 'Critical contradictions require immediate attention',
        affected_contradictions: scoredContradictions
    ...
    if (itemKindDistribution.decision > itemKindDistribution.entity * 2) {
      insights.push({
        ...
        description: 'High concentration of decision contradictions',
        affected_contradictions: scoredContradictions
   ...
   * Get item from cache
   */
  private getFromCache(key: string): { score: ScoreBreakdown; timestamp: number } | null {
   ...
   * Set item in cache
   */
  private setToCache(key: string, score: ScoreBreakdown): void {
    this.cache.set(key, {
      score,
      timestamp: Date.now(),
    });
