 ...
 */
function extractContentForTruncation(item: unknown): string | null {
  if (!item || typeof item !== 'object') {
    return null;
  }
 ...
 */
async function addTruncationMetadata(
  ...
  if (wasTruncated) {
    warnings.push(`${truncationDetails.length} items were truncated during storage`);
  }
