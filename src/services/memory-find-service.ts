...
export class MemoryFindService {
   ...
   * Simple text search
   */
  async searchText(text: string, options?: Partial<SearchQuery>): Promise<MemoryFindResponse> {
   ...
   * Semantic search
   */
  async semanticSearch(query: string, options?: Partial<SearchQuery>): Promise<MemoryFindResponse> {
    const searchQuery: SearchQuery = {
      query,
      mode: 'auto',
      ...options,
