...
export async function findRisks(
  ...
  return risks.map(risk => ({
    ...
    mitigation: Array.isArray(risk.mitigation_strategies) && risk.mitigation_strategies.length > 0 ?
      (typeof risk.mitigation_strategies[0] === 'string' ? risk.mitigation_strategies[0] : JSON.stringify(risk.mitigation_strategies[0])) : undefined,
...
export async function updateRisk(
  ...
  const result = await db.update('riskLog', { id: id }, {
      ...
      mitigation_strategies: data.mitigation ? [data.mitigation] : existing.mitigation_strategies || undefined,
