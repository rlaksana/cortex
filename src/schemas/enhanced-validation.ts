...
export const ScopeFilterSchema = z.object({
  ...
  branch: z.string().min(1, "Branch name is required").max(100, "Branch name too long").optional(),
  org: z.string().min(1, "Organization name is required").max(100, "Organization name too long").optional(),
}).strict();
...
export const SectionDataSchema = z.object({
  ...
  (data) => data.body_md || data.body_text || data.heading || data.title,
  {
...
export const DecisionDataSchema = z.object({
  ...
  status: z.enum(['proposed', 'accepted', 'deprecated', 'superseded'], {
    errorMap: () => ({ message: "Status must be one of: proposed, accepted, deprecated, superseded" })
  }),
  title: z.string()
    .min(1, "Decision title is required")
    .max(500, "Title cannot exceed 500 characters")
    .trim(),
  rationale: z.string().min(10, "Rationale must be at least 10 characters").max(50000, "Rationale too long"),
  ...
  (data) => data.status !== 'accepted' || data.rationale.length >= 50,
  {
...
export const EnhancedKnowledgeItemSchema = z.object({
  kind: z.enum([
    ...
    errorMap: () => ({ message: "Invalid knowledge kind. Must be one of the 16 supported types" })
  }),
  ...
  (item) => item.kind === 'section' ? item.scope : true,
  {
...
export const MemoryStoreRequestSchema = z.object({
  items: z.array(EnhancedKnowledgeItemSchema)
